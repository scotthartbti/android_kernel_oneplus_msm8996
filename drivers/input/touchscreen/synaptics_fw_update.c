...
static void parse_header(void)
{
	...
	memcpy(img->product_id, data->product_id, sizeof(data->product_id));
	...
	if (img->is_contain_build_info) {
		img->package_id = (data->pkg_id_msb << 8) | data->pkg_id_lsb;
	...
	dev_dbg(&fwu->rmi4_data->i2c_client->dev,
		...
		img->image_size, img->config_size);
	...
	img->lockdown_data = fwu->data_buffer +
	    FW_IMAGE_OFFSET - img->lockdown_block_count * fwu->block_size;
...
static int fwu_read_f34_queries(void)
{
	...
	retval = fwu->fn_ptr->read(fwu->rmi4_data,
				   fwu_get_address(OFFSET_BLOCK_SIZE), buf, 2);
	if (retval < 0) {
		dev_err(&i2c_client->dev,
...
static enum flash_area fwu_go_nogo(void)
{
	...
	if (img->is_contain_build_info) {
		...
		if (img->package_revision_id != ((pkg_id[3] << 8) | pkg_id[2])) {
			flash_area = MISMATCH;
			goto exit;
		}
	}
	...
	dev_dbg(&i2c_client->dev,
		...
		deviceFirmwareID, (unsigned int)imageFirmwareID);
			    ...
			    unsigned char command)
{
	...
	dev_info(&i2c_client->dev,
		 "%s: update %s %3d / %3d\n",
		 __func__, command_str, block_cnt, block_cnt);
#endif
	return 0;
}
